...
8. **TikTok slideshow import** (§5a) - genuinely low-risk given the existing `ImportProvider` extension point, but not blocking anything else, so it's fine last or interleaved with 6-7 as capacity allows. TikTok Shop product import (§5b) stays parked pending the user's own business decision, as it already was.
