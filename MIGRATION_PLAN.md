...
| **TikTok slideshow download** ("Downie" replacement) | Zero references to "Downie" or any video-download automation anywhere in the codebase. Entirely different domain from TikTok Shop product scraping - never attempted, never blocked. The `ImportProvider` Protocol's own docstring already names a future `TikTokSlideshowImporter` as the expected extension point. | **A clean, unbuilt extension point** - genuinely new work, but the architecture already anticipated it (§5). |
...
### 2. The four design principles, checked against reality
...
**(a) TikTok slideshow/post download** (the "Downie" replacement) - downloading the carousel images from a public TikTok post URL. **Untested, unblocked, and already anticipated** by the `ImportProvider` Protocol's own docstring. Proposed as a new `TikTokSlideshowImporter` implementing the existing `ImportProvider` Protocol, registered alongside `LocalFileImporter` in `importers/registry.py` - architecturally this is close to free, since the abstraction already exists and was designed for exactly this. **But it needs its own feasibility spike first** (confirm a real, ToS-respecting download path exists and works against real TikTok URLs), matching the exact discipline Phase 5.4 already used for TikTok Shop - "investigate first, build second" is this project's standing rule, not a one-off for that spike.
...
### 8. Creative Intelligence - consolidate the responsibility, not necessarily the tables

`CreativeFingerprint`, `MarketingAnalysis`, and `NarrativeStructure` already cover most of what's asked for: creative style/colour usage/visual hierarchy (fingerprint), strategy (marketing analysis), and hook/story/reveal/proof/cta structure (narrative structure, which is literally "hook structure" already implemented). Two real gaps: **object placement** and **emotional trigger** are more granular than anything captured today. Proposed: add these as new fields on `CreativeFingerprint.structured_json` (additive, no new table - it's already the per-slide visual-style artifact, and these are visual-style facts) rather than inventing a fourth artifact table that would need its own staleness/provenance wiring for no real benefit. "Creative Intelligence" becomes the *name* for the responsibility these three artifacts jointly hold, surfaced as one section in the Project-level assembled view (§3) - not a new pipeline stage.
...
3. **Sequencing recommendation**: implement `ProviderCapabilities` fully and correctly for the OpenAI adapter first (real, verifiable via the live-call discipline this project already uses), and treat "Nano Banana Lite" as a real adapter to build only once its actual API is being integrated - not speculatively designed against an API this document hasn't inspected. This mirrors the exact caution Product Lock v2 already applied to embeddings ("prove it before building for it").
4. Every other Protocol (OCR, vision, prompt/text generation) stays single-provider for now - multi-provider selection is only justified where there's an actual second candidate, per the standing 2026-07-22 directive to justify new infrastructure with the concrete blocker it removes.

### 11. Decision Engine

Entirely new - nothing today chooses a provider, reference set composition, creativity level, candidate count, or retry strategy; every one of those is currently either hardcoded or absent. Proposed as a new service (`app/services/decision_engine.py`), not a `SlideshowAnalysisStage` - like `image_validation_stage` before it, its unit of work (one generation attempt) doesn't fit the "operates on a whole Slideshow" Protocol shape. Given a Project/Slide/CreativeSpecification plus a quality-mode input (Fast/Balanced/Maximum Quality, §12), it produces a `GenerationPlan`: chosen provider (from §10's capability data - e.g. only a provider whose `supports_reference_images` is true is eligible once a Library exists), candidate count, creativity-level parameters passed to the Prompt Compiler, and - on a retry - what to change based on *why* the previous attempt failed (§13's Quality Engine output), not just "try again with the same inputs." This last point matters: a blind resample-and-hope retry is much weaker than one that reads the failure reason (e.g. identity kept failing → narrow the Reference Set to sharper images; creative fidelity was weak → try a different provider) - the Decision Engine's real value is in closing that loop adaptively, not just counting attempts.
...
The three genuinely new dimensions (Creative Fidelity, Image Quality, Text Quality) don't fit inside `ImageValidationResult`'s existing shape, which is deliberately scoped to per-field product-identity checks - forcing them in would either bloat one artifact past its single responsibility (a pattern this codebase has consistently avoided: OCR/Fingerprint/MarketingAnalysis are already separate artifacts, not one mega-row) or require reopening an ADR that's already been reviewed twice. Proposed instead: a new `QualityAssessment` entity (per `GeneratedImage` candidate) that **references** the existing `ImageValidationResult` for the Product Fidelity dimension and adds three new JSON columns for the others - `creative_fidelity_json` (composition/layout/hierarchy scoring, informed by §8's Creative Intelligence facts), `image_quality_json` (artifact/distortion/blur detection - a vision-provider call with its own dedicated schema, no Product Profile involved), `text_quality_json` (only populated when `text_strategy=ai_rewrite`; hook-preserved/readability/layout-fit scoring) - plus one computed `overall_confidence_score` and `accepted: bool`, following this project's standing rule that pass/fail is always computed in code from structured sub-scores, never asked of the AI as a bare boolean (same discipline `image_validation_stage.py` already uses for `passed`). Bundle products validate individually by construction - `QualityAssessment` rows key off the same `product_id`-per-appearance shape `ImageValidationResult` already uses, no new bundle-specific logic needed *for validation itself* (generation is the part that's actually bundle-constrained - see §14e).
...
- **Intelligence layer** (parallel, each derived independently from Evidence): Product Intelligence, Creative Intelligence, Text Intelligence.
...
3. **Decision Engine + Generation Engine candidate counts + basic retry loop** (§11-§12, §14) - proves Autonomous Quality end-to-end using the *existing* single Product Fidelity dimension, before Creative/Image/Text Quality exist. This is the same "prove the loop, then add capability" discipline the session's own standing directive already asks for.
4. **Quality Engine's new dimensions** (§13) - Creative Fidelity and Image Quality layer onto the now-working loop; Text Quality waits for step 6.
5. **Project-as-aggregation-root + Evidence Engine unification** (§3-§4) - foundational, but the existing Slideshow-centric workflow keeps working throughout, so this doesn't block steps 2-4 and can land in parallel once someone's available for it.
6. **Text Intelligence + Rendering Engine** (§9, §15) - biggest, riskiest, most novel; needs its own feasibility spike (matching the rendering-fidelity problem named in §15) before any schema work.
7. **TikTok slideshow import** (§5a) - genuinely low-risk given the existing `ImportProvider` extension point, but not blocking anything else, so it's fine last or interleaved with 5-6 as capacity allows. TikTok Shop product import (§5b) stays parked pending the user's own business decision, as it already was.
...
- *TikTok Slideshow Import - feasibility spike* - mirroring Phase 5.4's format exactly (investigate, report findings, then decide whether to build), for §5a.
...
- Run the TikTok slideshow-download feasibility spike (§5a) before promising the Import Pipeline in any user-facing form - the same investigate-first discipline that already saved real effort on TikTok Shop.
- Don't build the full N-provider Capability Framework speculatively (§10.3) - build `ProviderCapabilities` for OpenAI for real, and treat a second provider as real work only once it's actually being integrated.
...
Every bullet in the request's success-criteria list maps to a subsystem above: upload/TikTok-URL entry → Evidence Engine + Import Pipeline (§4-§5); optional product URLs → Evidence Engine's product-evidence path, reusing existing `ProductSourceAdapter`s (§4); text strategy choice → Text Intelligence (§9); automatic evidence analysis → the existing compute-on-read pipeline, extended (§2, §8); multiple candidates → Generation Engine (§12); automatic validation across three fidelity dimensions → Quality Engine (§13); automatic retry → §14; highest-quality upload-ready output with minimal effort → Rendering Engine's `FinalOutput` (§15), gated by the whole closed loop in §16. No success-criterion bullet is left uncovered by this design - but per the implementation order above (§17), several of them (Text strategy, automatic retry, upload-ready rendering) are the *last* pieces to land, not the first, because they're the least proven and highest-novelty parts of the design.
