...
**New tables:**
- `CanonicalProductReference` (see §3) - the new first-class, versioned selection artifact.
- `CanonicalProductReferenceImage` - a join table between `CanonicalProductReference` and `ProductReferenceImage` (see §3 for why a join table, not a JSON id snapshot, is the right shape here specifically).

**Changed tables (all additive - no column removed, no existing column's meaning changed):**
- `ProductReferenceImage` gains nullable scoring columns: `quality_score` (float), `quality_reasons_json` (list of short strings, e.g. `["sharp", "front-facing", "brand fully visible"]` - same "record the reasoning, not just the number" discipline `ImageValidationResult.field_checks_json` already established). Computed once per image by the Reference Acquisition Engine (§4), not recomputed on every read.
- `GeneratedImage` gains a nullable `canonical_product_reference_id` FK - which canonical-reference *version* a given generation actually used, for the same provenance/staleness reasoning `CreativeSpecification` already records for `product_lock_profile_id`/`creative_fingerprint_id`. Nullable because pre-this-phase rows genuinely have none (see §10).
- `ImageValidationResult` gains `identity_passed` (nullable Boolean) and `identity_checks_json` (nullable list, same `{field_name, preserved, reason}` shape `field_checks_json` already uses - identity checks are compared field-by-field the same way, just against image geometry/branding instead of text facts). Nullable for the same pre-this-phase-row reason. The existing `passed`/`field_checks_json` columns are **repurposed in meaning, not shape**: `passed` becomes "identity passed AND creative passed" (see §7), `field_checks_json` continues to mean the creative/Stage-2 checks specifically.

**No table is dropped. No table is renamed. `ProductLockProfile` and `ProductReferenceImage` keep their current shape and current writers (Product Lock Profile Stage, Product Isolation Stage, the URL-import download path) completely unchanged** - this ADR changes what *consumes* them and adds a new selection layer on top, not how they're produced. This is deliberate: every prior migration in this project used the same expand → backfill → switch-reads → contract discipline, and there is no reason to break that discipline for a change this consequential - if anything, less reason.

### 2. Database migration strategy

Two new tables (`create_table`, no batch mode needed - no ALTER on any existing table's constraints), one migration each or combined - final call is an implementation-time judgment, not a design one. Plus two small, genuinely additive `ALTER TABLE ADD COLUMN` migrations (`product_reference_images.quality_score`/`quality_reasons_json`, both nullable, no FK - no batch mode needed by this project's own precedent, e.g. `19b5c9909b3e`) and one FK-adding batch-mode migration each for `generated_images.canonical_product_reference_id` and the two new `image_validation_results` columns (batch mode required per this project's own established rule whenever a migration adds a column with a new FK constraint - `4b599e19a719`, `15210f8f4d89`).
...
**Backfill is the one genuinely open decision, not a mechanical one** (see §11's explicit call-out) - whether to retroactively create a `CanonicalProductReference` for every `Product` that already has `ProductReferenceImage` rows today, and if so, whether selection uses free classical heuristics only or a real (paid, one AI call per candidate image) vision-scoring pass. This is a real-money, real-scope decision the user should make explicitly when implementation is authorized, not something decided inside a migration script.

### 3. New `CanonicalProductReference` entity and relationships

**Deliberate refinement of the request's proposed shape, with reasoning:** the request lists `Reference Images` as a child of `CanonicalProductReference`, alongside OCR/Fingerprint/Embeddings/Attributes/etc. Read literally, that would mean copying or re-owning image data that `ProductReferenceImage` already owns. Instead, `CanonicalProductReference` should **select from and point at** the existing `ProductReferenceImage` rows, not duplicate them - identical in spirit to how `ProductLockProfile.reference_image_ids_json` already points at (rather than copies) the images it was built from. The difference here is that each canonical image needs its *own* rank/role/quality score, which a flat JSON id list can't carry - so this ADR proposes a real join table (`CanonicalProductReferenceImage`) instead of reusing the JSON-snapshot pattern, and calls out that deviation explicitly rather than silently picking a shape:

```
CanonicalProductReference (AnalysisArtifactMixin-shaped: id, is_current, created_at,
                            schema_version - but see note below on analysis_run_id)
├── product_id (FK, owned by Product - same ownership model as ProductLockProfile)
├── product_lock_profile_id (FK, nullable) — the "supporting metadata" snapshot, §6
├── quality_score (float) — the aggregate score of the selected set, not a per-image score
├── selection_method_json — how these images were chosen (heuristic v1 / vision-scored / manual override), for auditability
└── source_metadata_json — where the winning source came from (product_source_import id, slideshow id, or "user_upload")

CanonicalProductReferenceImage (join table, new)
├── canonical_product_reference_id (FK)
├── product_reference_image_id (FK) — the existing row, not a copy
├── role (e.g. "front", "45_degree", "side", "packaging", "close_up" — free string, same
│         "plain string not a hardcoded enum" discipline ProductSourceImport.source_type uses)
├── rank (int — display/priority order among the canonical set)
└── quality_score (float — this specific image's score; the parent's quality_score is an aggregate)
```

`OCR Results`, `Product Fingerprint`, and `Visual Embeddings` from the request's list are addressed individually, not folded in as owned children, because each is either already covered by an existing entity or is a genuinely new capability decision that shouldn't be silently bundled into a single schema:

- **`Structured Attributes`/`Dimensions`/`Materials`/`Colours`/`Packaging`/`Brand Metadata`**: this is exactly what `ProductLockProfile.structured_json` and the canonical `ProductProfile` (Phase 5.5) already are. `CanonicalProductReference.product_lock_profile_id` points at the current one - no duplication, no new vocabulary. This directly implements the request's own "Product Lock Role" section (§6 below).
- **`OCR Results`**: the request likely means packaging/label text read directly off the reference images (brand name, volume, ingredients printed on the product) - a different thing from the existing `OCRResult`, which reads marketing-copy text off a *creative* (headline/CTA/disclaimer). `ProductLockProfile.structured_json.labels_and_text` already captures a version of this today (vision-derived, not dedicated-OCR-derived - see `FakeVisionAnalysisProvider`'s canned shape). **Recommendation: do not build a second, parallel OCR pipeline.** If dedicated OCR precision on packaging text is genuinely wanted beyond what the vision model's `labels_and_text` already gives, reuse the existing `OCRProvider` capability (already built, zero new AI capability) via a new Product-scoped Stage - not a new provider, not a new entity, just a new consumer of code that already exists.
- **`Product Fingerprint`**: this project's `CreativeFingerprint` is explicitly about *marketing/creative style* (visual style, marketing objective, mood - see its schema), not product identity - reusing that name for product-geometry identity would collide two different concepts under one name. If a distinct "what does this product look like, structurally" summary is wanted beyond the Lock Profile's own `shape_and_proportions`/`distinguishing_features` fields, it can live as an additional `ProductLockProfile` field (Phase 5's revision #5 vocabulary discipline: extend an existing field before inventing a new entity) rather than a new "Fingerprint" concept that would be confusable with the existing one.
- **`Visual Embeddings`**: **the one genuinely new AI-capability question in this whole ADR**, and per this project's own established rule (the 2026-07-22 standing directive itself), new infrastructure needs a concrete blocker, not a "would be nice." The concrete use for embeddings would be *fast, cheap candidate scoring/dedup across many images without a vision-model call per pair* - valuable once a Product has dozens of candidate images (many slideshows × many products), but Phase 8's actual scale (one slide, one product) doesn't need it yet. **Recommendation: defer embeddings entirely for v1.** Stage 1 Identity Validation (§7) and quality scoring (§4) can both be done with the *existing* `VisionAnalysisProvider` capability - a side-by-side image comparison call, structurally identical to what Phase 8.4 already built, no new provider. This mirrors the exact reasoning Phase 8.2 used to defer a "Platform Rules" entity: don't build the general infrastructure before the specific need proves it's required. Embeddings become real future work (this ADR's own equivalent of the frozen catalogue ADR's "Non-goals" section) once real usage shows vision-call cost/latency for scoring is actually a bottleneck - not before.
...
They do not migrate in the sense of being transformed or replaced - **they stay exactly as they are, forever, as one input among several** `CanonicalProductReference` optionally points at via `product_lock_profile_id` (§3). No `ProductLockProfile` row's shape, meaning, or writer changes. The only "migration" is the backfill question in §2/§11: whether to retroactively build a `CanonicalProductReference` for products that already have a current `ProductLockProfile` + `ProductReferenceImage` rows, so they don't need a fresh acquisition run to benefit from this ADR. This is additive scaffolding around existing data, not a transformation of it - if the backfill is skipped entirely, nothing breaks; those products simply have no `CanonicalProductReference` until one is triggered (same "not generated yet" empty-state discipline every artifact in this app already uses).
...
- **If no `CanonicalProductReference` exists yet** for the product (a real, expected state for every product this ADR is rolled out to before its first acquisition run): Stage 1 is skipped, `identity_passed`/`identity_checks_json` stay `null` ("unknown," not "failed" - the same tri-state discipline `creative_specification_staleness`'s `_unknown()` already established for missing provenance), and Stage 2 runs exactly as it does today. This is what makes the whole ADR backwards-compatible in practice (§10) rather than a hard cutover.

### 8. API changes

- `GET /api/products/{id}/canonical-reference` - new, mirrors `GET /api/products/{id}/lock-profile`'s shape; 404 when none exists yet (same `handleOptional`-friendly convention Phase 8.5 just established).
- `POST /api/products/{id}/acquire-references` - new, explicitly triggers the Reference Acquisition Engine (§4). Background-task-based given realistic candidate volume (unlike Phase 8's deliberately-synchronous single-call endpoints).
- `POST /api/products/{id}/reference-images/upload` - new, Priority 3 (user uploads).
- `POST /api/products/{id}/canonical-reference/select` - new, the human-in-the-loop override this ADR's lifecycle section (§4) requires - lets a user manually choose which images are canonical rather than trusting the automatic score, and confirms an offered upgrade.
- `POST .../slides/{slide_id}/generate-image` (existing, Phase 8.3) - response gains `canonical_product_reference_id: string | null` on `GeneratedImageRead` (additive field, mirrors `creative_specification_id`).
...
- Existing `POST /api/products/{id}/source-import` (Phase 5.6) - **unchanged externally.** Internally, a successful import becomes one more trigger the Reference Acquisition Engine can react to (§4's lifecycle detection), but the endpoint's request/response contract does not change.

### 9. UI changes

- Product-level view gains a "Canonical Reference" panel: the selected images with their `role`/`quality_score`, an explicit "why was this chosen" surface (from `selection_method_json`), and a manual override control - matching the request's own emphasis that this becomes the product's primary visual identity, so a user should be able to see and correct it directly, not just trust an opaque score.
- When a higher-quality source is detected (§4's lifecycle), a non-blocking prompt offering to upgrade - never a silent swap, matching the catalogue ADR's established human-in-the-loop discipline for consequential product-identity decisions.
- The Blueprint modal's "Generated Image" section (Phase 8.5) gains a distinct **Identity** badge, shown above the existing creative Pass/Fail, so a user immediately sees *which kind* of failure occurred - "wrong product" reads completely differently from "wrong lighting," and conflating them in one badge would be a real regression from what Phase 8.5 just built. When no `CanonicalProductReference` exists yet, the section says so honestly ("Generated without a Canonical Reference - lower fidelity expected, text-only") rather than silently proceeding as if nothing changed - same "never silently degrade" instinct this session has applied consistently (e.g. Narrative Structure's forced `unclassifiable`, staleness's tri-state).

### 10. Backwards compatibility

Everything in §1-§9 above is additive by construction, and this section exists to state that as a testable claim, not just an intention:
- No existing table, column, endpoint, or response field is removed or renamed.
- Every new FK (`canonical_product_reference_id`, `product_lock_profile_id` on the new table) is nullable.
- Every new response field (`identity_passed`, `identity_checks`, `canonical_product_reference_id`) is nullable/optional, defaulting to `null`/absent for any row created before this ADR ships - a pre-existing `GeneratedImage`/`ImageValidationResult` from Phase 8.3/8.4 (including the two real rows already sitting in the dev DB from tonight's live verification) remains a fully valid, readable historical record exactly as it is today.
- `compile_generation_request`'s new parameter is optional with a safe default (no reference images → behaves exactly as it does today, `images.generate` not `images.edit`).
- The existing `POST /source-import`, `POST /generate-image`, `POST /validate` contracts are unchanged for any caller that doesn't opt into the new fields.
- `SLIDESHOW_STAGE_PIPELINE` is untouched - Reference Acquisition, like Image Generation/Validation before it, is deliberately not part of the automatic analysis pipeline, for the same real-cost-per-call reason Phase 8's architecture direction already established.
...
7. **Backfill (timing TBD, real cost decision):** whether/how to retroactively build `CanonicalProductReference` rows for existing products - proposed as its own explicit go/no-go decision after 9.1-9.5 are live-verified, not bundled into the schema work.
...
- **Cost and latency**: `images.edit` with reference images is a heavier call than `images.generate` (more input tokens, likely slower) - real, unmeasured until 9.3's live call; Phase 8.3's own real call already took ~53s text-only, so this needs headroom, not just an assumption it'll be similar.
...
- **Quality-scoring accuracy**: Tier 1 heuristics (resolution/file-size-as-sharpness-proxy) are crude and will misrank some real candidates; Tier 2 (vision-scored) is more accurate but costs real money per candidate image, and cost scales with how many candidates a product has (a 10-slide slideshow could mean 10 vision calls just to score crops from one import).
- **`images.edit` reference-image count/size limits**: not yet verified against the real API (unlike the size-mapping/model-name issues Phase 8.3 already found and fixed live) - a real unknown to resolve during 9.3, not assumed away here.
- **Embeddings deferral is a real, acknowledged bet**: acceptable at today's scale (one slide, few products), a genuine future need once candidate volume grows - deferring is the right call per this project's own "prove it before building for it" discipline, but it is a deferral, not a decision that embeddings are unnecessary forever.
...
- **Backfill cost is an open, real-money decision** (§2/§11) - explicitly not decided in this document, flagged for the user's own call before 9.1 ships, not assumed to be "score everything automatically."
- **Migration discipline risk is low, by design**: every change in §1-§10 is additive; the highest-risk part of this whole ADR is not the schema, it's the judgment calls in scoring/selection logic (§4) and the real cost of reference-image-conditioned generation (§12 above) - which is exactly why this is being proposed as a reviewed plan first, per the explicit instruction, rather than implemented directly the way Phase 8's own sub-phases were once their direction was set.
