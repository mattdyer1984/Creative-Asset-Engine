...
**(a) TikTok slideshow/post download.** *(Revision #3 reorders this: native browser automation is now the primary recommendation, tested directly; Downie 4 - Revision #2's finding, still fully valid - is demoted to a documented fallback below.)*
...
- *TikTok Slideshow Import (`PlaywrightTikTokImporter` primary, `DownieImportProvider` fallback) - implementation ADR* *(narrowed further in Revision #3 - no longer a "what's even possible" feasibility spike for either path, since both automation surfaces are now fully documented; this future ADR covers the native importer's implementation, the one remaining `imagePost`-shape confirmation, and the Downie fallback's implementation as a lower-priority parallel track)*, for §5a.
