...
Give `colour_roles` as a list of {role, colour}, where role is a name the design actually distinguishes (accent, body, muted...) and colour is a plain colour name such as dark-red or near-black. Use the roles this design really has, not a fixed list.

Give `text_roles` as a list of entries, one per role (headline, subhead, body, label, numeral, bullet...). Each entry has: role, family_class, weight (light/regular/medium/bold/black), italic (true/false), case (as-written/upper/lower/title), colour_role naming one of the colour roles above, alignment (left/center/right), size_ratio relative to the body text (body is 1.0), tracking, and line_height. Give every field for every role.

Give `size_scale` as a list of {name, ratio}, e.g. name "headline_to_body", ratio 2.9.
