...
from app.slideshow_stages.recreation_prompt_stage import (
    SlideRecreationPromptStage,
    _resolve_primary_appearance,
)
...
class _FakeAppearance:
    """
    Minimal stand-in for ProductAppearance - _resolve_primary_appearance
...
def test_resolve_primary_appearance_empty_list_returns_none():
    assert _resolve_primary_appearance([]) is None
...
def test_resolve_primary_appearance_single_appearance_returns_it():
    ...
    assert _resolve_primary_appearance([only]) is only
...
def test_resolve_primary_appearance_prefers_prominence_primary_over_creation_order():
    ...
    result = _resolve_primary_appearance([earlier_secondary, later_but_primary])
...
def test_resolve_primary_appearance_falls_back_to_earliest_created_when_none_marked_primary():
    ...
    result = _resolve_primary_appearance([later, earlier])
...
def test_resolve_primary_appearance_ties_among_primaries_broken_by_earliest_created():
    ...
    result = _resolve_primary_appearance([later_primary, earlier_primary])
...
def test_multi_product_slide_builds_prompt_around_the_primary_appearance(
    db_session, slideshow_with_product, monkeypatch
):
    """
    Phase 6.3: unlike Product Isolation/Lock Profile, a multi-product
    slide doesn't fail Recreation Prompt outright - it resolves to the
    appearance marked prominence == "primary" and builds the prompt
    around that product's Lock Profile, ignoring the secondary one.
    """
    ...
    assert result.succeeded is True
    db_session.refresh(slideshow_with_product)
    prompt = db_session.get(RecreationPrompt, slideshow_with_product.current_recreation_prompt_id)
    assert prompt.product_lock_profile_id == primary_profile.id
